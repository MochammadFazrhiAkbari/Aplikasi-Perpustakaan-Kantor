        ...
        if (auth()->user()) {
            ...
            if (auth()->user()->hasRole('peminjam')) {
                ...
                if ($peminjaman_lama->count() == 5) {
                    session()->flash('gagal', 'Buku yang dipinjam maksimal 5');
                } else {
                    ...
                    if ($peminjaman_lama->count() == 0) {
                        ...
                        $sisa = $buku->stok - 1;
                        DB::table('buku')->where('ini',$buku->id)->update([
                            "stok" => ($sisa)
                        ]);
                        session()->flash('sukses', 'Buku berhasil ditambahkan ke dalam keranjang stock buku adalah '.$sisa);
                    } else {
